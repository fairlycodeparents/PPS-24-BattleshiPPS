...
- distinguere le diverse tipologie di navi (`Frigate`, `Submarine`, `Destroyer`, `Carrier`), attraverso una `enum`, ognuna con una lunghezza 
...
- calcolare dinamicamente le celle occupate, fornite dal metodo `positions: Set[Position]`.
  ...
  In questo caso è stato utilizzata una `lazy val` (_call-by-need_), così che le posizioni vengano calcolate solo al 
  primo accesso e poi memorizzate. Questo evita ricalcoli inutili (dato che `Ship` è immutabile) e migliora l’efficienza.
  ```scala
...
Le navi vengono, quindi, istanziate direttamente a partire dal tipo, attraverso dei _factory methods_ definiti all’interno della `enum`.
...
In questo modo il codice client deve sempre gestire uno di questi casi, attraverso pattern matching, 
senza possibilità di stati imprevisti.
...
- validare la posizione scelta (`validateAttack`), cioè controllare che sia nel tabellone e non sia già stata attaccata:
  ...
  ```
  la validazione sfrutta `Option.when`, che in maniera concisa ed espressiva restituisce `Some` solo quando la condizione è verificata;
...
(posizione invalida) da casi di gioco (ad esempio un colpo già effettuato).
...
La creazione dei giocatori è centralizzata in una _factory_, 
...
Per i bot, `position` è `None`, mentre per il giocatore umano, `position` deve essere specificata.
...
```
- `TargetAlreadyHitStrategy`(_mixin_): non è una strategia autonoma, ma un modulo riutilizzabile 
...
- `PlayerTest` per lo sviluppo dei giocatori e la strategia di attacco:
  ...
  ritorno al comportamento casuale dopo l'affondamento),
    ...
    che itera sugli attacchi finché una nave non è affondata, evitando rischi di stack overflow.
...
In particolare, il mio contributo ha riguardato una prima semplice implementazione iniziale, per permettere all'utente
di posizionare le sue navi su una versione base della board e, nell'ultimo sprint, alcuni miglioramenti riguardanti l'usabilità, 
